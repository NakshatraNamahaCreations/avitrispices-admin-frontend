...
function Categories() {
  ...
  const handleCreateCategory = async () => {
    ...
    try {
        const response = await axios.post('https://api.nncwebsitedevelopment.com/api/categories', {
...
// useEffect to fetch the categories when the component mounts
useEffect(() => {
  fetchCategories();
}, []);
...
const handleEditCategory = (category) => {
  ...
  setNewCategoryName(category.category); // Populate the input with the current name
  setNewStatus(category.status); // Populate the input with the current status
  setShowTable(false); // Optionally hide the table
};
...
const handleUpdateCategory = async () => {
  ...
  try {
      const response = await fetch(`https://api.nncwebsitedevelopment.com/api/categories/${editCategory._id}`, {
          method: "PUT",
...
const handleDeleteCategory = async (categoryId) => {
  try {
      console.log("Deleting category with ID:", categoryId); // Log the category ID being deleted

      const response = await fetch(`https://api.nncwebsitedevelopment.com/api/categories/${categoryId}`, {
          method: 'DELETE',
      });
